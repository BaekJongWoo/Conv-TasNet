...
flags.DEFINE_bool("memory_limit", False, "Memory Limit")


def main(argv):
    W = int((FLAGS.T) * (FLAGS.L / 2))

    data = dataset.Dataset(E=FLAGS.E, M=FLAGS.M, C=FLAGS.C, W=W, dataset_path=FLAGS.dataset_path,
    ...
    while not epoch == FLAGS.epochs:
        ...
        history = model.fit(x, y, batch_size=FLAGS.M)
